...
if ~exist('data_extra_channels','var'), data_extra_channels = []; end
...
if ~exist('leftexclude','var')
    if ~isempty(settings) && isfield(settings,'leftexclude') && ~isempty(settings.leftexclude)
        leftexclude = settings.leftexclude;
    else
        leftexclude = 0;
    end
end
...
if pregauss > 0
    ...
    template_image = prefilter_data(template_image, pregauss, size(template_image), 0);
end
...
if nargout > 7
    ...
    data_corrected(:,:,:,1) = resample_corrected(data,xpixelposition,ypixelposition,[nrows_template ncols_template],frame_list);
    for ecind = 1:n_extra_channels
        data_corrected(:,:,:,1 + ecind) = resample_corrected(data_extra_channels(:,:,:,ecind),xpixelposition,ypixelposition,[nrows_template ncols_template],frame_list);
    end
end
if nargout > 8
...
for kk = 1:numel(idata_inputvars)
    if ismember(idata_inputvars{kk}, {'data_extra_channels'}) && eval(['isempty(' idata_inputvars{kk} ')'])
        continue; %this variable can be empty
    end
...
function [p,corr,iter,mask] = align_frame_to_template(data,template,est_p,xgrad,ygrad,frac,blockind,basex, basey, minpointsperblock, max_iter, move_thresh, haltcorr, nblocks)
...
base_mask = reshape(1:numel(basex), size(basex)) <= framew * frameh; %mask that removes "extra pixels" when the number of pixels is not an integer multiple of (pixels / block)

frac = blockorder(frac, blocksize, framew, frameh);
data_bm = blockorder(data, blocksize, framew, frameh);
...
templatew = size(template,2); templateh = size(template,1);
...
mask = base_mask & x > 1 + eps & x < templatew - eps & y > 1 + eps & y < templateh - eps;
...
wI = x_frac_comp .* ((y_frac_comp) .* template(matind) + ...
    y_frac .* template(matindp1)) + ...
    x_frac .* ((y_frac_comp) .* template(matindpth) + ...
...
while iter <= max_iter && corr < haltcorr
    ...
    delta_p(:) = 0;
    delta_p(params_used) = H(params_used, params_used) \ v(params_used);
...
defaultsettings = struct('move_thresh',0.010,'corr_thresh',0.75,'max_iter',120,'scanlinesperparameter',2,'pregauss',pregauss,'haltcorr',0.995,'bidirec',isbidirec,'leftexclude',leftexclude);
if isempty(settings)
    ...
    for u = 1:numel(fn)
        if ~isfield(settings, fn{u})
            ...
            %should check that structure fields have valid values FIXME
        end
    end
end
...
function d = resample_corrected(data,xpixelposition,ypixelposition,template_image_size,frame_list)
